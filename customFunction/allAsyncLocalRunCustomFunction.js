...
const shuffleArray = async (array) => {
  ...
  while (currentIndex != 0) {
    ...
    randomIndex = await floorRND(currentIndex);
    currentIndex--;
...
const shuffleObjectWithNumberKeys = async (obj) => {
...
const getMinOverlapNeeded = async (
  dvColPosArrayPositions,
...
const getPositionWithMinOverlap = async (
  ...
  let iteration = 0;
  let rndPosFromLeft = -1;
  let numPosAvailable = dvColPosSetAvailable.size;

  if (numPosAvailable > 0) {
    const rnd = await floorRND(numPosAvailable);
    rndPosFromLeft = Array.from(dvColPosSetAvailable.values())[rnd];
    ...
    loop1: while (iteration < maxIterationsBeforePickingAnyRND) {
      ...
      rndPosFromLeft = await rndPos(dvImgWidth);
...
const getPosWithOverlapAllowed = async (
 ...
 */
const addPosWithAllowableOverlap = async (
  dvColPosSet,
  dvColVisualizePositionsArray,
  dvColPosArrayPositions,
  dvColPosSetAvailable,
  ...
  if (allowOverlap) {
    overlap = await getMinOverlapNeeded(
      dvColPosArrayPositions,
      maxPositionsWithoutOverlap,
      overlap
    );
  ...
  ({
    ...
    dvColPosArrayPositions,
    dvColPosSetAvailable,
  } = await getPositionWithMinOverlap(
    maxIterationsBeforePickingAnyRND,
    dvColVisualizePositionsArray,
    dvColPosArrayPositions,
    dvColPosSetAvailable,
...
const addPosWithOverlap = async (
  ...
  let rndPosFromLeft = await rndPos(dvImgWidth);
  ...
  ({
    ...
    dvColVisualizePositionsArray,
    dvColPosArrayPositions,
  } = await getPosWithOverlapAllowed(
    numPosWithoutCompleteOverlap,
    dvColPosSet,
    rndPosFromLeft,
    dvColVisualizePositionsArray,
...
const generateDVColPosArrays = async (numOfDVs, dvImgWidth) => {
  ...
  while (posCreated < numOfDVs) {
    if (posCreated > 100) {
      ...
      ({
        ...
        dvColPosSetAvailable,
        noLuck,
      } = await addPosWithAllowableOverlap(
        dvColPosSet,
        dvColVisualizePositionsArray,
        dvColPosArrayPositions,
        dvColPosSetAvailable,
 ...
 */
const generateCodes = async (numOfCodes) => {
  const codes = [];
...
const getImgToUseAtPos = async (
  ...
  const rndChanceToUseOhImg = await floorRND(chanceForOh);
...
const getImgToUseAtPosHorizontal = async (
  chanceForOh,
  ohs,
  ohhs,
  ones,
...
const fillRenderings2D = async (
  ...
  for (let i = 0; i < numOfDVs; i++) {
    ...
    ({ image, image2 } = await getImgToUseAtPosHorizontal(
      chanceForOh,
      ohs,
      ohhs,
      ones,
...
const getImgOptions = async (theme) => {
  const mes = images.me;
  ...
  if (theme === "seeingDouble") {
    ohs = await shuffleArray(mes);
    ones = await shuffleArray(as);
    twos = await shuffleArray(bs);
  } else if (theme === "ahhhhhh") {
    ohs = await shuffleArray(mees);
    ones = await shuffleArray(cs);
    twos = await shuffleArray(ds);
  } else if (theme === "racing") {
    ohs = await shuffleArray(meees);
    ohhs = await shuffleArray(meeees);
    ones = await shuffleArray(fs);
    oness = await shuffleArray(ffs);
    twos = await shuffleArray(gs);
    twoss = await shuffleArray(ggs);
...
const combineCodesAndPosArrayAndImgs = async (
  numOfDVs,
  codes,
  dvColPosArrayPositions,
  dvColVisualizePositionsArray,
  ...
  // init
  let renderings = {};
  const initRND = await floorRND(numOfDVs);
  ...
  if (Object.values(imgOptions).length > 3) {
    ...
    renderings = await fillRenderings2D(
      numOfDVs,
      initRND,
      chanceForOh,
      ohs,
    ...
    renderings = await fillRenderings(
      numOfDVs,
      initRND,
      chanceForOh,
      ohs,
...
module.exports = a = async (params = localParams) => {
  const numOfDVs = await Number(params.numDVs);
  const dvImgWidth = await Number(params.dvImgWidth);
  ...
  let {
    ...
    positionsOverlapping,
    maxPositionsWithoutOverlap,
  } = await generateDVColPosArrays(numOfDVs, dvImgWidth);

  const codes = await generateCodes(numOfDVs);

  const { code, renderings } = await combineCodesAndPosArrayAndImgs(
    numOfDVs,
    codes,
    dvColPosArrayPositions,
    dvColVisualizePositionsArray,
